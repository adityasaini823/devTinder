...
const userSchema = Joi.object({
  ...
  dob: Joi.date()
    .max(dayjs().subtract(1, "year").toDate())
    .required()
    .messages({
...
router.post("/login", async (req, res) => {
  try {
    ...
    res.cookie("refreshToken", refreshToken, {
      maxAge: 7 * 24 * 60 * 60 * 1000, // 7 Days
      httpOnly: true,
      secure: true, // Use secure cookie only in production
    });
  ...
  try {
    ...
    jwt.verify(
      ...
      async (err, decoded) => {
        ...
        const mainuser = await User.findOne({ username: decoded.username });
        if (!mainuser) {
        ...
        // console.log(user);
        res.json({ accessToken: newAccessToken, user: mainuser });
      }
    );
  } catch (err) {
    res.status(500).json({
