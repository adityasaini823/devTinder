...
router.get("/",userValidator,async(req,res)=>{
    try{
         // Fetch matched user IDs where current user is involved
        ...
        // Fetch users excluding self and matched users
        const users = await User.find({
            _id: { $nin: [req.user._id, ...matchedUserIds] }, // Exclude self & matched users
        });
...
router.post("/accept/:request_id",userValidator,async(req,res)=>{
    try{
        const frndReq=await Swipe.findByIdAndUpdate(req.params.request_id,{$set:{
            status:"accepted"
        }},{ new: true });
...
router.post("/reject/:request_id",userValidator,async(req,res)=>{
    try{
        const frndReq=await Swipe.findByIdAndUpdate(req.params.request_id,{$set:{status:"rejected"}},{new:true});
        const io = req.app.get("io");
